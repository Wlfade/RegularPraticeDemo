...
@interface KKChatContactMsgCell (){
...
@implementation KKChatContactMsgCell
...
-(void)layoutSubviews {
    ...
    //1.img
    self.imgView.frame = CGRectMake(leftSpace, 6, _imgViewWidth, _imgViewWidth);
    
    //2.title
    CGFloat titleW = msgContentW - self.imgView.right - 8 - rightSpace;
    CGFloat titleH = 20;
    self.nameLabel.frame = CGRectMake(self.imgView.right+8, 23, titleW, titleH);
    ...
    //5.author
    CGFloat tagH = msgContentH-self.grayLine.bottom;
    self.tagLabel.frame = CGRectMake(leftSpace+2, self.grayLine.bottom, msgContentW-leftSpace-rightSpace-2, tagH);
}
...
- (void)setDataModel:(RCMessageModel *)model {
    ...
    //2.内容
    KKChatContactMsgContent *content = (KKChatContactMsgContent *)self.model.content;
    
    //2.1 img
    [self.imgView sd_setImageWithURL:Url(content.imgUrl) placeholderImage:Img(@"userLogo_defualt")];
