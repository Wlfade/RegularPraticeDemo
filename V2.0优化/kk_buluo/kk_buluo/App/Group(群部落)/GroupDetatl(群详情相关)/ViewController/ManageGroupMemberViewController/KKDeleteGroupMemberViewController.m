//
//  KKDeleteGroupMemberViewController.m
//  kk_buluo
//
...
@interface KKDeleteGroupMemberViewController ()<UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate, DSectionIndexViewDataSource, DSectionIndexViewDelegate>
...
@implementation KKDeleteGroupMemberViewController
#pragma mark - INIT
...
- (void)setRightButton {
    ...
    NSString *titleStr;
    if (self.selectedUserList.count > 0) {
        titleStr = [NSString stringWithFormat:@"删除(%zd)", [self.selectedUserList count]];
...
- (void)viewDidLoad {
    ...
    [self setNaviBarTitle:@"删除群成员"];
    
    /// 创建
    _doneButton = [DGButton btnWithFontSize:[ccui getRH:14] title:@"删除" titleColor:[UIColor colorWithRed:206/255.0 green:206/255.0 blue:206/255.0 alpha:1.0] bgColor:[UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1.0]];
    ...
    [_doneButton addTarget:self action:@selector(deleteButtonAction:) forControlEvents:UIControlEventTouchUpInside];
...
/**
 requestContactData: 进行通讯录好友的请求
 */
- (void)requestContactData {
    /// 把数据源传给需要作为搜索源的数组
    [self.userSimpleList addObjectsFromArray:self.dataArray];
    //// 去排序, 取值.
    self.allKeys = [KKContactDealTool sortGroupMemberArrayWithPinYin:self.dataArray][@"allKeys"];
    self.allFriends = [KKContactDealTool sortGroupMemberArrayWithPinYin:self.dataArray][@"infoDic"];
...
- (void)deleteButtonAction:(UIButton *)obj {
    if ([obj.titleLabel.text isEqualToString:@"删除"]) {
        return;
    }
    [self requestDeleteGroupMemberData];
}

/**
 requestDeleteGroupMemberData: 进行删除群成员的请求
 */
- (void)requestDeleteGroupMemberData {
    NSMutableArray *list = [NSMutableArray array];
    for (KKGroupMember *user in self.selectedUserList) {
        [list addObject:user.userId];
    }
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setValue:@"GROUP_USER_REMOVE" forKey:@"service"];
    [params setValue:[KKUserInfoMgr shareInstance].userId forKey:@"authedUserId"];
    [params setValue:[list componentsJoinedByString:@","] forKey:@"userIdList"];
    ...
    [[CC_HttpTask getInstance] post:[KKNetworkConfig currentUrl] params:params model:nil finishCallbackBlock:^(NSString *str, ResModel *resModel) {
        if (str) {
            ...
            [CC_NoticeView showError:@"删除群成员成功"];
