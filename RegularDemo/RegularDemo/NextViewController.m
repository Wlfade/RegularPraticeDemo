...
- (void)viewDidLoad {
    ...
    NSString *summary = [NSString stringWithFormat:@"比如服务端给的数据的形式是这样的:转发的带有表情的summary<emotion image_name=\"hongdan.png\" image_alt=\"hongdan\">hongdan</emotion>"];
...
- (NSMutableArray *)remakeTheSummaryString{
    ...
    /**
     ...
     options 正则匹配选项
     */
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"<(emotion)(.*?)(</emotion>)" options:NSRegularExpressionAllowCommentsAndWhitespace error:nil];
    /*
    ...
    for (NSTextCheckingResult *item in result) {
        ...
        if ([imgHtml rangeOfString:@"image_alt=\""].location != NSNotFound) {
            //将字符串分为两段
            tmpArray = [imgHtml componentsSeparatedByString:@"image_alt=\""];
        } else if ([imgHtml rangeOfString:@"image_alt="].location != NSNotFound) {
            tmpArray = [imgHtml componentsSeparatedByString:@"image_alt="];
        }
