...
- (UITableView *)didJoinInTableView {
    if (!_didJoinInTableView) {
        _didJoinInTableView  = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - (STATUSBAR_ADD_NAVIGATIONBARHEIGHT + 50 + 5))];
...
- (UITableView *)willJoinInTableView {
    if (!_willJoinInTableView) {
        _willJoinInTableView  = [[UITableView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT - (STATUSBAR_ADD_NAVIGATIONBARHEIGHT + 50 + 5))];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.didJoinInTableView) {
        ...
        cell.myRecommend = self.successArray[indexPath.row];
        return cell;
    }else {
        
        KKMyRecommendCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cellIDWill"];
        cell.myRecommend = self.willJoinInArray[indexPath.row];
        return cell;
    }
}
...
- (void)getMyRecommendData:(NSString *)status {
    if ([status isEqualToString:@"SUCCESS"]) {
        ...
        [self.willJoinInArray removeAllObjects];
    }
    WS(weakSelf)
    ...
    [[CC_HttpTask getInstance] post:[KKNetworkConfig currentUrl] params:params model:nil finishCallbackBlock:^(NSString *str, ResModel *resModel) {
        ...
        if ([status isEqualToString:@"SUCCESS"]) {
            ...
            [weakSelf.successArray addObjectsFromArray:[KKMyRecommend mj_objectArrayWithKeyValuesArray:responseDic[@"list"]]];
            if (weakSelf.successArray.count > 0) {
                [weakSelf.didJoinInTableView reloadData];
            }
        }else {
            
            [weakSelf.willJoinInArray addObjectsFromArray:[KKMyRecommend mj_objectArrayWithKeyValuesArray:responseDic[@"list"]]];
