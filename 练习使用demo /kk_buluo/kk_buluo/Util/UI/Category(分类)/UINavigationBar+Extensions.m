...
- (void)xtSetBackgroundColor:(UIColor *)backgroundColor
{
    if (self.overlay == nil) {
        ...
        if (iPhoneX) {
            self.overlay = [[UIView alloc] initWithFrame:CGRectMake(0, -44, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) + 44)];
        }else {
...
{
    [[self valueForKey:@"leftViews"] enumerateObjectsUsingBlock:^(UIView *view, NSUInteger i, BOOL *stop) {
        view.alpha = alpha;
    }];
    
    [[self valueForKey:@"rightViews"] enumerateObjectsUsingBlock:^(UIView *view, NSUInteger i, BOOL *stop) {
        view.alpha = alpha;
    }];
